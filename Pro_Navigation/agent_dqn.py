...
from Pro_Navigation.model import QNetwork
...
class Agent():
    ...
    def learn(self, experiences, gamma):
        ...
        # print(self.qnetwork_target(next_states)) # shape:[BATCH_SIZE,4]
        ...
        # print(self.qnetwork_local(states)) # shape:[BATCH_SIZE,4]
        Q_expected = self.qnetwork_local(states).gather(1, actions)  # shape:[BATCH_SIZE,1]

        # Compute loss
        loss = F.mse_loss(Q_expected, Q_targets) # 用Q估计值和Q目标计算均方差损失函数，都为列向量
        # Minimize the loss
