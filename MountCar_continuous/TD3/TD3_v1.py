...
from BipedalWalker.TD3.TD3_model import Actor,Critic
...
class TD3_V1(object):
    def __init__(self, state_dim, action_dim, max_action, env):
        self.actor_local = Actor(state_dim, action_dim, max_action).to(device)
    ...
    def step(self,time_step,state, action, next_state, reward, done):
        self.memory.add((state, action, next_state, reward, done))

        if self.memory.__len__()>=BATCH_SIZE:
            self.train(time_step, batch_size=BATCH_SIZE, discount=GAMMA, tau=TAU, policy_noise=NOISE,
                        noise_clip=CLIP, policy_freq=POLICY_FREQ)


    def train(self, time_step, batch_size, discount, tau, policy_noise,noise_clip, policy_freq):
        ...
        if time_step % policy_freq == 0:
            ...
            actor_loss = -self.critic_local.Q1(state, action_pred).mean()
            ...
            for param, target_param in zip(self.critic_local.parameters(), self.critic_target.parameters()):
                target_param.data.copy_(tau * param.data + (1 - tau) * target_param.data)

            for param, target_param in zip(self.actor_local.parameters(), self.actor_target.parameters()):
                target_param.data.copy_(tau * param.data + (1 - tau) * target_param.data)


    def save(self, directory,filename):
        torch.save(self.actor_local.state_dict(), '%s/%s_actor.pth' % (directory, filename))
        torch.save(self.critic_local.state_dict(), '%s/%s_critic.pth' % (directory, filename))
